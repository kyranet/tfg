...
const loginUNED = async (req, res) => {
    ...
    try {
        // user
        const userBD = await dao_usuario.obtenerUsuarioSinRolPorEmail(email);
        let user = {
            uid: userBD.id,
            email: userBD.correo,
            rol: userBD.rol,
            password: userBD.password,
            password_2: userBD.password,
            nombre: userBD.nombre,
            apellidos: userBD.apellidos,
            universidad: userBD.nombreUniversidad,
            facultad: userBD.facultad,
            areaConocimiento: userBD.area_conocimiento,
            sector: userBD.sector,
            nombreSocioComunitario: userBD.nombre_socioComunitario,
            origin_img: userBD.origin_img,
            origin_login: userBD.origin_login,
            terminos_aceptados: userBD.terminos_aceptados,
            telefono: userBD.telefono,
            url: userBD.url,
            mision: userBD.mision
        }
        if (!userBD) {
            return res.status(404).json({
                ok: false,
