...
const { readFileFromS3, uploadFileToS3, deleteFileFromS3 } = require('../helpers/s3');
...
const puedeObtenerFichero = (upload, peticionario) => {
    if(upload.tipo === 'usuarios' && upload.campo === 'avatar') { return true; }

    // revisar permisos
    if(upload.tipo === 'iniciativas') { return true; }
    if(upload.tipo === 'partenariados') { return true; }
    if(upload.tipo === 'proyectos') { return true; }
    return false;
}
...
const obtenerFichero = async(req, res = response) => {
    ...
    const fichero_por_defecto = ficheroPorDefecto(res, req.params.default_type || '');

    // util por ejemplo para las fotos de avatares
    if(!upload_id) {
        return fichero_por_defecto ? res.sendFile(fichero_por_defecto) : res.status(400).json({
            ok: false,
            msg: 'No existe el fichero',
        });
    }

    const upload = await Upload.findById(upload_id).then((upload) => { return upload; }).catch((err) => {});
    ...
    if( !puedeObtenerFichero(upload, req.current_user ? req.current_user.uid: null) ) {
    ...
    if(process.env.FILE_STORAGE === 's3') {
        ...
        if( fs.existsSync(upload.path)) {
            res.setHeader('Content-Disposition', 'attachment; filename=' + upload.client_name || upload.nombre);
...
const subirFichero = (req, res = response) => {
    try {
...
const borrarFichero = async(req, res = response) => {
    try {
        ...
        // validar permisos de usuario
        if( (upload.creador != req.current_user.uid) && (req.current_user.rol !== ROL_GESTOR)) {
            return res.status(401).json({
                ok: false,
        ...
        if(upload.tipo === 'usuarios' && upload.campo === 'avatar') {
            await Usuario.findByIdAndUpdate(upload.tipo_id, {origin_img: ''}, { new: true });
        }

        if(upload.tipo === 'iniciativas' && upload.campo === 'default') {
            await Iniciativa.findByIdAndUpdate(upload.tipo_id, {imagen: ''}, { new: true });
        }
