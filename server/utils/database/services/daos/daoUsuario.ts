...
async function insertarOficinaAps(usuario: tOficinaAps): Promise<number> {
    try {
      ...
      await knex('oficinaaps')
        .insert({ id: idUsuario, datos_personales_Id: datosPersonalesId });
...
async function insertarProfesorInterno(usuario: tProfesorInterno): Promise<number> {
    try {
      ...
      await knex('profesor_interno')
        .insert({ id: idF, datos_personales_Id: result[0] });
      ...
      await knex('titulacionlocal_profesor')
      ...
      await knex('areaconocimiento_profesor')
...
export async function insertarEstudianteExterno(usuario: TEstudianteExterno): Promise<number> {
    try {
      ...
      const values = await knex("universidad")
        .select("id")
        .where("nombre", "like", `%${usuario.nombreUniversidad}%`);
...
export async function insertarProfesorExterno(usuario) {
  try {
    ...
    const resultDatosPersonales = await knex("datos_personales_externo")
      .insert({
      ...
      }, 'id'); // Asegúrate de que 'id' se devuelve correctamente después de la inserción
    ...
    const valuesUniversidad = await knex("universidad")
      .select("id")
...
export async function obtenerUsuarioSinRolPorEmail(email: string): Promise<any|null> {
  try {
      ...
      let idInternos = await knex("datos_personales_interno")
          .where({ correo: email })
          .select("id");
      ...
      let idExternos = await knex("datos_personales_externo")
          .where({ correo: email })
          .select("id");
...
export async function obtenerAreasConocimientoUsuario(id: number): Promise<any[]> {
  try {
    const id_areas = await knex("areaconocimiento_profesor")
    ...
    const areas_conocim = await knex.select("nombre", "id")
...
async function obtenerProfesorInternoPorDatosPersonales(datosPersonalesId: number): Promise<tProfesorInterno> {
  try {
    ...
    const profesorInterno = await knex("profesor_interno")
                                    .where({ datos_personales_Id: datosPersonalesId })
                                    .select("*");
    if (profesorInterno.length === 0) return null;
...
async function obtenerAreasConocimientoDelProfesor(idProfesor: number): Promise<string[]> {
  ...
  const areas = await knex.select("nombre").from("area_conocimiento").whereIn("id", idAreas.map(a => a.id_area));
...
async function obtenerTitulacionesDelProfesor(idProfesor: number): Promise<string[]> {
  ...
  const titulaciones = await knex.select("nombre").from("titulacion_local").whereIn("id", idTitulaciones.map(t => t.id_titulacion));
...
async function obtenerEstudianteInternoPorDatosPersonales(datosPersonalesId: number): Promise<TEstudianteInterno | null> {
  try {
    const estudianteInterno = await knex("estudiante_interno")
                                      .where({ datos_personales_Id: datosPersonalesId })
                                      .select("*");
    if (estudianteInterno.length === 0) return null;
    ...
    const usuario = await obtenerUsuario(id);
    const datos = await obtenerDatosPersonalesInterno(datosPersonalesId);
    const titulacion = await knex("titulacion_local")
...
async function obtenerEstudianteExternoPorDatosPersonales(datosPersonalesId: number): Promise<TEstudianteExterno | null> {
  try {
    const estudianteExterno = await knex("estudiante_externo")
                                      .where({ datos_personales_Id: datosPersonalesId })
                                      .select("*");
    if (estudianteExterno.length === 0) return null;
    ...
    const usuario = await obtenerUsuario(id);
    const datos = await obtenerDatosPersonalesExterno(datosPersonalesId);
    const universidad = await knex("universidad")
...
async function actualizarAdmin(usuario:TAdmin) :Promise<number> {
  ...
  try {
    ...
    const resDatosPersonales = await knex("datos_personales_interno")
      .where("id", usuario.id)
      .update({
        nombre: usuario.nombre,
        apellidos: usuario.apellidos,
...
async function actualizarSocioComunitario(usuario:TSocioComunitario): Promise<number> {
  try {
    ...
    const rel = await knex("datos_personales_externo")
    ...
    const rel2 = await knex("socio_comunitario")
      .where("id", usuario.id)
      .update({
        sector: usuario.sector,
        nombre_socioComunitario: usuario.nombre_socioComunitario,
...
async function actualizarEstudianteExterno(usuario:TEstudianteExterno): Promise<number> {
  try {
    ...
    const resDatosPersonales = await knex("datos_personales_externo")
      .where("id", usuario.id) 
      .update({
        nombre: usuario.nombre,
        apellidos: usuario.apellidos,
    ...
    const resEstudianteExterno = await knex("estudiante_externo")
      .where("id", usuario.id)
      .update({
        titulacion: usuario.titulacion,
...
async function actualizarProfesorExterno(usuario: tProfesorExterno): Promise<number> {
  try {
    ...
    const rel2 = await knex("profesor_externo")
      .where("id", usuario.id)
      .update({
...
async function actualizarEstudianteInterno(usuario: TEstudianteInterno): Promise<number> {
  try {
    ...
    const rel = await knex("datos_personales_interno")
      .where("id", usuario.id)
      .update({
        nombre: usuario.nombre,
        apellidos: usuario.apellidos,
    ...
    const rel2 = await knex("estudiante_interno")
      .where("id", usuario.id)
      .update({
        titulacion_local: usuario.titulacion_local,
      });
