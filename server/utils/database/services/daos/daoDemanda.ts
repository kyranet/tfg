...
async function obtenerAreaServicio(id_anuncio: number): Promise<AreaServicio[]> {
    try {
        ...
        const id_areas = await knex('areaservicio_anuncioservicio')
            .where({ id_anuncio: id_anuncio })
            .select('id_area');

        // Consultamos en la tabla area_servicio donde el id está en la lista de ids obtenidos anteriormente
        const areas: AreaServicio[] = await knex.select('*')
            .from('area_servicio')
            .whereIn('id', id_areas.map(area => area.id_area));
...
export async function obtenerDemandaPorAreaServicio(id_areaServicio: number): Promise<DemandaServicio[]> {
    try {
        const id_demandas = await knex('areaservicio_iniciativa')
...
export async function obtenerDemandaPorNecesidadSocial(id_necesidadSocial: number): Promise<DemandaServicio[]> {
    try {
        ...
        const demandas: DemandaServicio[] = await knex.select('*')
...
export async function obtenerDemandaServicio(id_demanda: number): Promise<DemandaServicio> {
    try {
        ...
        const necesidad_social = await knex('necesidad_social')
...
async function crearAnuncio(anuncio: AnuncioServicio): Promise<number> {
    try {
        ...
        const [id_anuncio] = await knex('anuncio_servicio')
            .insert({
...
export async function obtenerTodasDemandasServicio(limit: any, offset: any, filters: any): Promise<TDemandaServicio[]> {
    try {
...
async function eliminarDemanda(id_demanda: number): Promise<void> {
    try {
        const resultado = await knex('demanda_servicio')
            .where('id', id_demanda)
            .del();
...
async function obtenerTitulacionLocal(id_demanda: number): Promise<TitulacionLocal[]> {
    try {
        ...
        const id_titulaciones = await knex('titulacionlocal_demanda')
            .where({ id_demanda })
        ...
        const titulaciones = await knex('titulacion_local')
            .select('id', 'nombre')
            .whereIn('id', titulacionesIds);
...
async function obtenerAreaServicioDemanda(demanda: number): Promise<AreaServicioAnuncio[]> {
    try {
        const areasServicio = await knex<AreaServicioAnuncio>('areaservicio_anuncioservicio')
            .where({ id_anuncio: demanda });
