...
async function crearAnuncio(anuncio: TAnuncioServicio): Promise<number> {
    try {
        const [idAnuncio] = await knex("anuncio_servicio").insert({
...
export async function obtenerTodasOfertasServicio(limit: number, offset: number, filters: string): Promise<TOfertaServicio[]>{
    ...
    try {
        ...
        for (let oferta of datos_ofertas) {
            ...
            const asignaturasObjetivo = await knex("asignatura")
...
async function obtenerAsignaturaObjetivo(id_oferta: number): Promise<String[]> {
    try {
        const nombres = await knex("asignatura")
            .where({ id_oferta })
            .select("nombre");
...
export async function obtenerAnuncioPorAreaServicio(id_areaServicio: number): Promise<TAnuncioServicio[]> {
    try {
        const id_anuncios = await knex("areaservicio_anuncioservicio")
...
async function obtenerIdsAreas(nombre_areas: string[]): Promise<number[]> {
    try {
        const ids = await knex("area_servicio")
...
async function obtenerCreadorOferta(id: number): Promise<number> {
    try {
        const creador = await knex("oferta_servicio")
            .where({ id })
            .select("creador");
