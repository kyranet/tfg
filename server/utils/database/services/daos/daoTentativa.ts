...
async function obtenerIniciativa(id: number): Promise<TIniciativa | null> {
  ...
  async function obtenerTodasIniciativas(): Promise<TIniciativa[]> {
    try {
      ...
      const areas = await knex("areaservicio_iniciativa")
      ...
      const transfersIniciativas = todasIniciativas.map(iniciativa => {
        const areasServicio = areas
          .filter(area => area.idIniciativa === iniciativa.id)
          .map(area => area.area);
  ...
  async function actualizarIniciativa(iniciativa: TIniciativa): Promise<number> {
    try {
      ...
      await knex("areaservicio_iniciativa")
        .where({ id_iniciativa: iniciativa.id })
        .del();
