...
async function obtenerNotificaciones(idUser: number): Promise<Notificacion[]> {
	try {
		const resultado = await knex('notificaciones').where({ idDestino: idUser }).select('*');
...
async function obtenerOfertaAceptadaServicio(idNotificacion: number): Promise<Notificacion | undefined> {
	try {
		const resultado = await knex('notificaciones')
			.join('ofertaaceptada', 'notificaciones.id', '=', 'ofertaaceptada.idNotificacion')
...
export async function obtenerNotificacionOfertaAceptada(idnotificacion: any): Promise<Notificacion> {
	try {
		const resultado = await knex('ofertaaceptada').select('*').where({ idNotificacion: idnotificacion });
...
async function obtenerNotificacionAceptacionAceptadaPorIdPartenariado(idPartenariado: number): Promise<Notificacion> {
	try {
		const resultado = await knex('aceptacionaceptada').select('*').where({ idPartenariado: idPartenariado });
		return resultado;
	} catch (err) {
...
async function obtenerNotificacionDemandaRespaldadaPorIdPartenariado(idPartenariado: number): Promise<Notificacion[]> {
	try {
		const resultado = await knex('demandarespalda').select('*').where({ idPartenariado: idPartenariado });
		return resultado;
	} catch (err) {
...
async function FinalizarPendienteNotificacion(idNotificacion: number): Promise<void> {
	try {
		await knex('notificaciones').where({ id: idNotificacion }).update({ pendiente: false }); // Se usa bool en lugar de 0
	} catch (err) {
...
async function borrarNotificacion(idNotificacion: number): Promise<void> {
	try {
		await knex('notificaciones').where({ id: idNotificacion }).delete();
	} catch (err) {
...
export async function crearNotificacionAceptadacionAceptada(
	notificacion: Notificacion,
	idNotificacionOferta: any,
	idPartenariado: any
): Promise<any> {
	let idNotificacion;
	try {
...
export async function notificarPartenariadoRellenado(notificacion: Notificacion): Promise<void> {
	...
	try {
		...
		const notificacionFinalizado = await obtenerNotificacionAceptacionAceptadaPorIdPartenariado(notificacion.idPartenariado).catch(() =>
			obtenerNotificacionDemandaRespaldadaPorIdPartenariado(notificacion.idPartenariado)
		);
